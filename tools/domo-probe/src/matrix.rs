...
async fn device_login_status(root_pem: &[u8], f: &Fixture) -> Result<u16> {
    let (status, _) = device_login(root_pem, f).await?;
    Ok(status)
}

/// The REST device login, returning the token it issued.
async fn device_login_token(root_pem: &[u8], f: &Fixture) -> Result<String> {
    let (status, body) = device_login(root_pem, f).await?;
    anyhow::ensure!(status == 200, "device login for the fixture returned {status}");
    let auth: DeviceAuth = serde_json::from_str(&body).context("decoding the device token")?;
...
async fn device_login(root_pem: &[u8], f: &Fixture) -> Result<(u16, String)> {
