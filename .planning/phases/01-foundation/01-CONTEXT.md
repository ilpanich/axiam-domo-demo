...
- **D-11:** **The root is RSA-4096** (locked by constraint, not preference):
  ...
  - The root is still generated offline and imported via BYOK (D-10).
  - **Tenant signing CAs are Ed25519 by choice, not by force** — amended after research (C-3): AXIAM *can* generate RSA-4096 CAs (`KeyAlgorithm::Rsa4096` in `axiam-pki/src/crypto.rs:71-76`, since commit `bacc92a1d`, present in every `v1.0.0-beta*` tag), and `CreateIntermediateCaRequest` takes a caller-chosen `key_algorithm`. Ed25519 is chosen for instant keygen on the Pi and consistency with Ed25519 leaves; RSA-4096 is a one-field fallback if D-27's broker check fails. `axiam/docs/pki/README.md` is stale on this point — log it as a dogfooding finding.
...
- **D-20:** **One service account per (service, tenant)**, e.g. `mgmt@lakeside`, `mgmt@summit`, `twin@lakeside`, `twin@summit`, and sim-host accounts in Phase 4.
  ...
  - **Amended after research (C-5):** service-account tokens (`aud: axiam:m2m`) are rejected by **every** REST management route — each handler takes `AuthenticatedUser` and `check_user_aud_and_parse_jti` fails M2M audiences (`axiam-api-rest/src/extractors/auth.rs:951-992, 1080-1113`). `POST /authz/check` does accept service accounts. So Phase 1 still creates `mgmt@`/`twin@` service accounts and certs (valid for CheckAccess and device-style auth), and **Phase 2's Management Platform needs a per-tenant *user* principal for management calls** — the per-tenant admin user Phase 1 must create anyway (see D-37) is the candidate. Log as a dogfooding finding.
...
- **D-23:** The **test device is `tools/domo-probe` (Rust)**. It:
  ...
  3. logs in over mTLS at `POST /api/v1/auth/device` — **amended after research (C-6):** the Rust SDK has no device-login method (`src/rest/auth.rs` exposes only `login`, `verify_mfa`, `refresh`, `logout`), so the probe builds its mTLS client with the SDK (`with_client_cert`) and hand-rolls this one call. Log as a dogfooding finding.
  4. connects with rumqttc and rustls
  5. publishes and subscribes on its own `domo/{tenant}/{device}/…` topic, where **`{device}` is the device's service-account UUID** — the same value as the cert CN, the JWT `sub` and the MQTT username, so the auth backend authorizes a topic with no registry lookup
...
- **D-24:** **MQTT identity binding — resolved by research (C-2), strengthened fallback.** The HTTP backend genuinely never sees the certificate (`rabbit_auth_backend_http.erl` passes only `username`, `password`, `vhost`, `client_id`), but the **broker** closes the gap: `mqtt.ssl_cert_client_id_from = distinguished_name` rejects any CONNECT whose `client_id` ≠ the cert subject DN (`rabbit_mqtt_processor.erl` → `RC_CLIENT_IDENTIFIER_NOT_VALID`), independently of `ssl_cert_login`. So:
  - The broker enforces the cert at TLS: `verify_peer` + `fail_if_no_peer_cert` (CA bundle per D-26).
  - CSR subject is `CN=<device-service-account-UUID>`; CONNECT uses `client_id = "CN=<sa-uuid>"`, `username = <sa-uuid>`, `password` = the AXIAM JWT.
  - The backend validates the JWT and checks `sub == username` **and** `client_id == "CN=" + username`. The cert↔subject binding is therefore enforced end-to-end, covering the mismatched-cert and cross-tenant negatives in D-25.
  - Residual risk to log as a finding: a forged-CN cert issued by another tenant's admin combined with a stolen JWT.
...
- **D-26:** **Broker config:**
  ...
  - A **`30-mqtt.conf` conf.d fragment** sets: the MQTTS listener on 8883, TLS 1.3, `verify_peer`, `fail_if_no_peer_cert`, `mqtt.ssl_cert_client_id_from = distinguished_name` (D-24), `auth_backends.1 = internal` (AXIAM's own AMQP user keeps working), `auth_backends.2 = http` (devices, pointed at the Twin).
  - **CA bundle = the organization root, amended after research (C-1).** The MQTT listener takes only a port and inherits the broker-wide `ssl_options.*` shared with AXIAM's AMQPS 5671 listener, so a tenant-CA bundle is not expressible per-listener; tenant CAs also don't exist before the broker starts and are re-created on every reset (D-10). The root is stable and pre-dates first boot. **Tenant separation moves to the HTTP backend** (D-24: JWT `tenant_id` + `sub`/`client_id` binding), not the TLS trust store.
...
- **D-37:** **Two consequences of the broker and API shapes, added after research (C-1, C-4, and the SDK tenant-header gap):**
  - **AXIAM's own AMQP client cert is infrastructure, offline root-signed** — a documented exception to PKI-03's "every device and service client cert is issued by AXIAM". With broker-wide `fail_if_no_peer_cert`, AXIAM must present a client cert *before* AXIAM is able to issue one. AXIAM supports it (`AXIAM__AMQP__TLS__CLIENT_CERT_PATH`/`CLIENT_KEY_PATH`, PKCS#8 PEM — `axiam-amqp/src/config.rs:58-68`). Record it in the dogfooding findings as an exception, not a gap.
  - **`domo-bootstrap` needs a per-tenant admin user.** The Rust SDK never sends `X-Axiam-Tenant` (the server's `ACTIVE_TENANT_HEADER`, `extractors/auth.rs:200`), so an org-level principal cannot act inside a tenant through the SDK. Bootstrap therefore: does org-scoped work (tenants, BYOK import, signing CAs) with the SDK as super-admin; hand-rolls the ~4 calls per tenant that create a tenant admin user (mirroring `axiam/scripts/e2e-bootstrap.sh`); then does all tenant-scoped work through a second SDK client logged in as that tenant admin. Leaf certs inherit `tenant_id` from the acting user, so this is also what makes PKI-03 work. Credentials go to `.secrets/` (D-13) and are the candidate principal for Phase 2 (D-20). Log the missing tenant-header support as a dogfooding finding.
...
- Which per-tenant credential the Twin uses for each tenant. **How it validates device JWTs is settled by research (C-7):** the SDK's gRPC client wraps only `check_access`/`batch_check`, so the Twin uses the SDK-native `token::JwksVerifier` (local EdDSA verify against `/oauth2/jwks`, with `expect_tenant_id` and `expect_audience("axiam:m2m")`). The missing `validate_token` wrapper is a minor dogfooding finding.
